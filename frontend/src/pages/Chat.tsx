...
export default function Chat() {
  ...
  const loadSessions = async () => {
    try {
      const res = await fetch('http://rpi4.netbird.selfhosted:3000/api/chat/sessions')
  ...
  const loadMessages = async (sessionId: number) => {
    try {
      const res = await fetch(`http://rpi4.netbird.selfhosted:3000/api/chat/sessions/${sessionId}/messages`)
      const data = await res.json()
      setMessages(data)
    } catch (error) {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(`http://rpi4.netbird.selfhosted:3000/api/chat/sessions/${currentSession.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          message: userMessage,
  ...
  const deleteSession = async (sessionId: number) => {
    ...
    try {
      const res = await fetch(`http://rpi4.netbird.selfhosted:3000/api/chat/sessions/${sessionId}`, {
        method: 'DELETE'
      })
