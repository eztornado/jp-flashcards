...
async function callLLM(messages, model, apiKey, baseUrls) {
    try {
        ...
        if (isOllama) {
            ...
            const response = await axios.post(`${baseUrls.zAI}/chat/completions`, {
                model: model,
                messages: messages,
                temperature: 0.7,
                max_tokens: 500,
        ...
        const isOllama = !!OLLAMA_HOST;
        console.error(`[LLM Error] ${isOllama ? "Ollama" : "zAI GLM"}:`, error.response?.data || error.message);
        if (error.response) {
...
async function callZAIGLM(messages) {
...
app.get("/api/kanji/random", (req, res) => {
    ...
    if (!row)
        return res.status(404).json({ error: "No kanji found" });
...
app.post("/api/kanji", (req, res) => {
    ...
    const stmt = db.prepare("INSERT INTO kanji (kanji, onyomi, kunyomi, translation) VALUES (?, ?, ?, ?)");
...
app.put("/api/kanji/:id", (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const { kanji, onyomi, kunyomi, translation } = parsed.data;
    const stmt = db.prepare("UPDATE kanji SET kanji=?, onyomi=?, kunyomi=?, translation=? WHERE id=?");
...
app.get("/api/quiz/translation", (req, res) => {
    const mode = req.query.mode || "jp-to-es"; // 'jp-to-es' o 'es-to-jp'
...
app.post("/api/import", upload.single("file"), (req, res) => {
    try {
        ...
        const insertStmt = db.prepare(`INSERT INTO words (kanji, romaji, translation) VALUES (?, ?, ?)
...
app.get("/api/chat/test", async (req, res) => {
    ...
    try {
        ...
        const testResponse = await callLLM(testMessages, isOllamaConfigured ? OLLAMA_MODEL : ZAI_MODEL, isOllamaConfigured ? undefined : ZAI_API_KEY, {
            zAI: ZAI_BASE_URL,
        });
        res.json({
            configured: true,
...
app.post("/api/chat/sessions/:id/messages", async (req, res) => {
    ...
    try {
        ...
        const userStmt = db.prepare("INSERT INTO chat_messages (session_id, role, content) VALUES (?, ?, ?)");
        userStmt.run(sessionId, "user", message);
        // Obtener historial de la conversación
        const history = db
            .prepare("SELECT role, content FROM chat_messages WHERE session_id = ? ORDER BY created_at ASC")
            .all(sessionId);
        // Obtener palabras aleatorias del vocabulario para contexto
        const randomWords = db
            .prepare("SELECT kanji, romaji, translation FROM words ORDER BY RANDOM() LIMIT 10")
            .all();
        const vocabContext = randomWords
            .map((w) => `${w.kanji} (${w.romaji}): ${w.translation}`)
            .join("\n");
        ...
        const assistantStmt = db.prepare("INSERT INTO chat_messages (session_id, role, content) VALUES (?, ?, ?)");
